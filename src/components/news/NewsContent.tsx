...
/**
 * Categories filter - static, no loading needed.
 */
 ...
 */
function FeaturedArticle({ articles }: { articles: NewsArticle[] }) {
  const featuredArticle = articles.find(article => article.featured);
  if (!featuredArticle) return null;

  const imageUrl = featuredArticle.image || 'https://placehold.co/800x600/f4a261/white?text=News+Update';
...
/**
 * News card with intersection observer animation.
 */
function NewsCard({ article }: { article: NewsArticle }) {
  ...
  return (
    <article
      ref={ref}
      className={`relative rounded-card overflow-hidden bg-white transition-transform duration-[400ms] hover:-translate-y-2 fade-up ${isVisible ? 'visible' : ''}`}
    >
...
/**
 * All articles grid with loading skeleton.
 */
function AllArticles({ articles }: { articles: NewsArticle[] }) {
 ...
 */
function ArchiveSection({ articleCount }: { articleCount: number }) {
  return (
    <section className="section bg-bone">
      <div className="container">
          ...
          {['January 2024', 'December 2023', 'November 2023'].map((month, index) => (
            <Card key={month} hover className="text-center p-6">
              ...
              <p className="text-sm opacity-50 mb-4">
                {index === 0 ? `${articleCount} articles` : '0 articles'}
              </p>
...
/**
 * News content wrapper with Suspense boundary.
 */
export default function NewsContent({ articles }: { articles: NewsArticle[] }) {
  return (
    <>
      ...
      <ArchiveSection articleCount={articles.length} />
    </>
  );
}
