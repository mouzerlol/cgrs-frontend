...
export function MonthNavigationBar({
  ...
  return (
    <div className={cn('flex items-center justify-between mb-sm px-xs w-full pb-xs', className)}>
      <button
        onClick={onPrevMonth}
        className="flex items-center justify-center w-8 h-8 border border-sage rounded-lg bg-transparent text-forest cursor-pointer transition-all duration-[200ms] ease-out-custom hover:bg-sage-light hover:border-forest focus:outline-none focus:shadow-[0_0_0_2px_theme(colors.terracotta.DEFAULT)]"
